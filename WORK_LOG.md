...
## 2026-08-03: 로컬 구동 블로커 전수 점검

### 배경
저장소 정리 커밋(`67bb628`) 직후, "지금 이 프로젝트를 구동하려면 무엇을 고쳐야 하는가"를 확인할 필요가 있었다.
특히 정리 과정에서 코드 5개를 삭제했기 때문에 컴파일이 깨졌을 가능성을 배제해야 했다.
기존 `docs/0722ready.md`는 기능 결함 위주라 환경·포트·프레시 클론 재현성은 다루지 않는다.

### 작업 내역

#### 1. 3개 축 병렬 조사
- BE(Spring Boot) — 빌드 설정, 전체 YAML, 삭제 코드 영향, 라이브 프로세스 상태
- FE(Android + Vite) / AI(FastAPI) — 의존성, env, 하드코딩 엔드포인트
- 인프라·문서 — compose, db, monitoring, exec, docs

#### 2. 실제 머신 상태와 대조 검증
조사 결과를 설치된 툴체인·가동 중인 컨테이너·점유 포트와 직접 대조했다.
`docker ps`로 5432/6379가 다른 프로젝트에 점유된 것을 확인했고,
`curl /actuator/health`로 BE가 현재 미기동 상태임을 확인했다.

#### 3. 문서화
`docs/local-run-blockers.md` 신규 작성. 블로커를 심각도별로 분류했다.
- A. 하드 블로커 9건 — 기동 자체가 불가
- B. 설정값 필요 6건 — 기동은 되나 기능이 죽음
- C. 구조적 결함 10건 — 재현성 훼손
- D. 모니터링(범위 밖) 4건
- E. 확인했고 문제 없는 항목 6건 — 재조사 낭비 방지용

### 결과

**코드는 깨져 있지 않다.** 삭제된 5개 클래스는 참조가 0곳이고 191개 `com.ssafy.tax7i.*` import가 전부 해소된다.
테스트 308개 통과 기록도 정리 커밋 이후 시각이었다. 막고 있는 것은 환경과 설정이다.

가장 중요한 발견 3가지:
1. `docker-compose.yml:6,24`가 바인딩하는 5432/6379를 `dailybrief-*` 컨테이너가 이미 점유 중 → `compose up` 즉시 실패.
   현재 떠 있는 `tax7i-postgres`/`tax7i-redis`는 compose가 아니라 수동 기동 결과(55432/56379)라 파일과 실제가 어긋나 있다.
2. `application.yaml:4-5`의 `spring.config.import`가 **프로필과 무관하게 무조건** 동작해,
   gitignore된 `application-local.yaml`의 하드코딩 `localhost:5432`가 `${DB_PORT}`를 덮어쓴다.
   `README.md:73`이 안내하는 `DB_PORT` 지정이 실제로는 먹지 않는다.
3. `AES_ENCRYPTION_KEY`/`JWT_SECRET`은 설정 전체에서 기본값이 없는 유일한 2개인데
   이를 공급하는 `application-local.yaml`이 미추적이다 → 프레시 클론은 기동 불가.

부수 확인: Claude 모델 ID `claude-opus-4-8`, `claude-haiku-4-5-20251001` 둘 다 현재 유효. 변경 불필요.

### 남은 작업
- 실행 방식(하이브리드 / 전부 compose / 전부 로컬) 결정 필요. A-1·A-2의 수정 방향이 여기서 갈린다
- 이번 작업은 조사·문서화까지이며 코드·설정 수정은 포함하지 않았다

### 변경된 파일
| 파일 | 변경 |
|------|------|
| `docs/local-run-blockers.md` | 신규 |
| `WORK_LOG.md` | 본 항목 추가 |
